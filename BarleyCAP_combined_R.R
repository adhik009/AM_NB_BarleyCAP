setwd("/Users/aniladhikari/Google Drive/TCAP/TCAP_combined")  #Set WD

library(rrBLUP)
geno <- read.table("TCAP_combined_genotype.hmp.txt",header=TRUE,as.is=TRUE,check.names=FALSE)
...
pheno <- read.table("TCAP_combined_pheno.txt",header=TRUE,as.is=TRUE,check.names=FALSE)
...
#P+K or K only model
#look at inflation
P.K<-GWAS(pheno, geno[,-2], fixed="Env", K=NULL, n.PC=2, min.MAF=0.05, n.core=1, P3D=TRUE, plot=TRUE)
...
P.K[hit,]
write.table(P.K,"TCAP_combined_emma.result.txt", sep="\t")
